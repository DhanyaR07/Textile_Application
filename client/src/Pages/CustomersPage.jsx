...
function CustomersPage() {
    ...
    const fetchCustomers = async () => {
        try {
            setLoading(true);
            const res = await fetch('https://textile-backend-jhm4.onrender.com/api/customers');
    ...
    const handleUpdateCustomer = async () => {
        try {
            ...
            const res = await fetch(`https://textile-backend-jhm4.onrender.com/api/customers/${editingCustomer.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(sanitized)
            });
    ...
    const deleteCustomer = async (id) => {
        ...
        try {
            const res = await fetch(`https://textile-backend-jhm4.onrender.com/api/customers/${id}`, { 
                method: 'DELETE' 
            });
            const data = await res.json();
            if (data.success) {
