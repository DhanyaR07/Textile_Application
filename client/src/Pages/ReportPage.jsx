...
function ReportPage() {
    ...
    const deleteOrderBundle = async (invoiceNo) => {
        try {
            const res = await fetch(`[https://textile-backend-jhm4.onrender.com](https://textile-backend-jhm4.onrender.com)/api/orders/${encodeURIComponent(invoiceNo)}`, { method: 'DELETE' });
            const data = await res.json();
            if (data.success) {
    ...
    const viewEntireBill = async (invoiceRecord) => {
        ...
        try {
            const res = await fetch(`[https://textile-backend-jhm4.onrender.com](https://textile-backend-jhm4.onrender.com)/api/invoices/details/${encodeURIComponent(targetInvoiceNo)}`);
