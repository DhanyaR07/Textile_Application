...
function ProductsPage() {
    ...
    const fetchProducts = async () => {
        try {
            setLoading(true);
            const res = await fetch('https://textile-backend-jhm4.onrender.com/api/products');
    ...
    const handleUpdateProduct = async () => {
        try {
            ...
            const res = await fetch(`https://textile-backend-jhm4.onrender.com/api/products/${encodeURIComponent(productIdentifier)}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(values)
            });
    ...
    const deleteProduct = async (itemKey) => {
        try {
            const res = await fetch(`https://textile-backend-jhm4.onrender.com/api/products/${encodeURIComponent(itemKey)}`, { 
                method: 'DELETE' 
            });
            const data = await res.json();
            if (data.success) {
